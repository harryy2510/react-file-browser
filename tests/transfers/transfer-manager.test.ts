...
type UploadPartArgs = Parameters<NonNullable<FileBrowserAdapter['uploadPart']>>[0]
...
describe('TransferManager', () => {
	test('runs simple uploads, reports progress, and arms beforeunload while active', async () => {
		const upload = vi.fn((_path: string, file: File, opts?: FileBrowserUploadOptions): Promise<FileNode> => {
	...
	test('passes conflict resolution to simple uploads', async () => {
		const upload = vi.fn(
			(path: string, file: File, opts?: FileBrowserUploadOptions): Promise<FileNode> =>
				Promise.resolve({
					path: opts?.onConflict === 'keep-both' ? path.replace('.bin', ' (1).bin') : path,
					name: file.name,
					kind: 'file',
					size: file.size
				})
		...
		expect(manager.getUpload('upload-1')?.result?.path).toBe('/demo (1).bin')
	})
	...
	test('uses multipart capabilities and resumes from completed parts', async () => {
		...
		expect(manager.getUpload(id)?.completedParts.map((part) => part.partNumber)).toEqual([1, 2])
		...
		expect(window.localStorage.getItem('rfb-transfer-test')).toContain('"status":"completed"')
	})
	...
	test('allocates a keep-both path before starting multipart uploads', async () => {
		...
		expect((await adapter.list('/')).items.map((item) => item.path).sort()).toEqual(['/big (1).bin', '/big.bin'])
	})
	...
	test('does not restore expired or stale completed downloads', () => {
		...
		expect(manager.getSnapshot().downloads.map((download) => download.id)).toEqual(['current'])
	...
	test('uses signed URLs for single-file downloads', async () => {
		const signedUrl = vi.fn(() => Promise.resolve('https://files.example/report.pdf'))
		const adapter: FileBrowserAdapter = {
			list: vi.fn(),
			createFolder: vi.fn(),
			delete: vi.fn(),
	...
	test('creates a real client zip from signed URLs', async () => {
		...
		const signedUrl = vi.fn((path: string) => Promise.resolve(`https://files${path}`))
		const adapter: FileBrowserAdapter = {
			list: vi.fn(),
			createFolder: vi.fn(),
			delete: vi.fn(),
