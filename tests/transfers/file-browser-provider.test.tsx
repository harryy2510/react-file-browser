...
function SnapshotProbe() {
  ...
  return (
    <output aria-label="Upload status">
      {snapshot.uploads.at(0)?.status ?? "none"}
    </output>
  );
}
...
describe("FileBrowserProvider", () => {
  ...
  test("uses localStorage persistence by default for restored uploads", () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Resume uploads" }),
    ).toHaveTextContent("Resume 1 upload?");
  });
  ...
  test("persists default provider uploads to localStorage", async () => {
    ...
    await waitFor(() =>
      expect(
        window.localStorage.getItem("react-file-browser-transfers"),
      ).toContain("/demo.bin"),
    );
  });
  ...
  test("floating widget pauses, resumes, and cancels upload transfers", async () => {
    ...
    expect(
      await screen.findByRole("complementary", { name: "Transfers" }),
    ).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", { name: "Pause upload demo.bin" }),
    );
    await waitFor(() =>
      expect(screen.getByLabelText("Upload status")).toHaveTextContent("paused"),
    );

    await user.click(
      screen.getByRole("button", { name: "Resume upload demo.bin" }),
    );
    await waitFor(() => expect(upload).toHaveBeenCalledTimes(2));
    await user.click(
      screen.getByRole("button", { name: "Cancel upload demo.bin" }),
    );
    await waitFor(() =>
      expect(screen.getByLabelText("Upload status")).toHaveTextContent(
        "cancelled",
      ),
    );
  ...
  test("floating widget dismisses a ready download after opening it", async () => {
    ...
    expect(
      screen.queryByRole("complementary", { name: "Transfers" }),
  ...
  test("floating widget groups recursive folder uploads", async () => {
    ...
    await waitFor(() =>
      expect(manager.getUpload("upload-1")?.status).toBe("completed"),
    );
    ...
    expect(
      screen.getByText("1 of 2 files, 1 folder created"),
    ).toBeInTheDocument();
