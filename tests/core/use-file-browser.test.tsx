...
describe("useFileBrowser", () => {
  ...
  test("optimistically creates, renames metadata, deletes, and rolls back failed delete", async () => {
    ...
    expect(
      result.current.items.find((item) => item.path === "/alpha.txt")?.name,
    ).toBe("Alpha renamed.txt");

    act(() => result.current.selectOnly("/zeta.txt"));
    await act(async () => {
      await result.current.deleteSelected();
    });
    expect(result.current.items.map((item) => item.path)).not.toContain(
      "/zeta.txt",
    );
    ...
    await act(async () => {
      await expect(rollback.result.current.deleteSelected()).rejects.toThrow(
        "delete failed",
      );
    });
    expect(rollback.result.current.items.map((item) => item.path)).toContain(
      "/zeta.txt",
    );
  });
  ...
  test("gates clipboard paste by adapter capabilities", async () => {
    ...
    expect((await full.list("/docs")).items.map((item) => item.path)).toContain(
      "/docs/alpha.txt",
    );
    ...
    await act(async () => {
      await expect(minimalHook.result.current.pasteInto("/")).rejects.toThrow(
  ...
  test("moves the current selection to a destination folder", async () => {
    ...
    expect((await adapter.list("/docs")).items.map((item) => item.path)).toContain(
      "/docs/alpha.txt",
    );
  });
  ...
  test("keeps successful paths removed after partial bulk move failure", async () => {
    ...
    await act(async () => {
      await expect(result.current.moveSelectedTo("/docs")).rejects.toThrow(
  ...
  test("uploads with explicit conflict resolutions", async () => {
    ...
    expect(result.current.items.map((item) => item.path)).toContain(
      "/alpha (1).txt",
    );

    await act(async () => {
      const skipped = await result.current.uploadFiles(
        [textFile("alpha.txt", "skip")],
        { onConflict: "skip" },
      );
