...
export function defineFileBrowserAdapterContract({ name, createAdapter }: AdapterContractFactory) {
	describe(`${name} adapter contract`, () => {
		...
		test('returns signed object URLs for uploaded files', async () => {
			...
			await expect(adapter.signedUrl('/notes.txt')).resolves.toMatch(/^blob:nodedata:/)
		})
		...
		test('renames display metadata without changing immutable paths', async () => {
			...
			await adapter.upload('/quarterly-report.pdf', textFile('quarterly-report.pdf', 'pdf', 'application/pdf'))
		...
		test('copies files to a destination folder with new storage keys', async () => {
			...
			expect(paths((await adapter.list('/archive')).items)).toEqual(['/archive/notes.txt'])
			expect(paths((await adapter.list('/')).items)).toEqual(['/archive', '/notes.txt'])
		})
		...
		test('moves files and folders recursively when the capability is present', async () => {
			...
			expect(paths((await adapter.list('/archive/docs')).items)).toEqual(['/archive/docs/readme.txt'])
		})
	})
}
