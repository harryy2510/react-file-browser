...
const textFile = (name: string, text = name) =>
...
function fakeDirectoryEntry(
  name: string,
  entries: Array<FakeFileEntry | FakeDirectoryEntry>,
): FakeDirectoryEntry {
  return {
    isFile: false,
    isDirectory: true,
...
describe("FileBrowser", () => {
  test("renders toolbar, grid/list toggle, selection details, and preview modal", async () => {
    ...
    expect(screen.getByRole("dialog", { name: /Preview/ })).toHaveTextContent(
      "quarterly-report.pdf",
    );
  });
  ...
  test("centers the empty details panel state", async () => {
    ...
    expect(screen.getByText("No item selected").parentElement).toHaveClass(
      "text-center",
    );
  });
  ...
  test("selection action buttons use a consistent icon treatment", async () => {
    ...
    for (const name of [
      ...
      expect(
        within(toolbar).getByRole("button", { name }).querySelector("svg"),
      ).not.toBeNull();
    }
  });
  ...
  test("clipboard actions announce simple feedback", async () => {
    ...
    expect(
      screen.getByRole("status", { name: "Clipboard status" }),
    ).toHaveTextContent("Copied 1 item");
  });
  ...
  test("announces folder state and selection count for screen readers", async () => {
    ...
    expect(screen.getByRole("status")).toHaveTextContent(
      "Files ready. 3 items. 0 selected.",
    );
    await user.click(screen.getByText("hero-banner.jpg"));
    expect(screen.getByRole("status")).toHaveTextContent(
      "Files ready. 3 items. 1 selected.",
    );
  });
  ...
  test("downloads the selected item from the details panel", async () => {
    ...
    const signedUrl = vi.fn(() =>
      Promise.resolve("https://files.example/hero-banner.jpg"),
    );
    ...
    await user.click(screen.getByText("hero-banner.jpg"));
    await user.click(
      screen.getByRole("button", { name: "Download hero-banner.jpg" }),
    );
  ...
  test("loads a signed URL for file previews", async () => {
    ...
    const adapter = await adapterWithFiles();
    const signedUrl = vi.fn(() =>
      Promise.resolve("https://files.example/preview/hero-banner.jpg"),
    );
    Object.defineProperty(adapter, "signedUrl", {
      value: signedUrl,
    });
  ...
  test("grid selection surface fills the scrollable file area", async () => {
    ...
    expect(screen.getByRole("grid", { name: "Files" })).toHaveClass(
      "min-h-full",
    );
  });
  ...
  test("keeps the new folder dialog open on name conflicts", async () => {
    ...
    expect(dialog).toHaveTextContent(
      "A file browser entry already exists at /assets",
    );
    expect(
  ...
  test("collapses middle breadcrumbs for deep paths", async () => {
    ...
    render(
      <FileBrowser
        adapter={adapter}
        initialPath="/workspace/clients/acme/briefs"
      />,
    );
    ...
    expect(
      within(breadcrumb).getByRole("button", { name: "workspace" }),
    ).toBeInTheDocument();
    expect(
      within(breadcrumb).queryByRole("button", { name: "clients" }),
    ).not.toBeInTheDocument();
    expect(
      within(breadcrumb).getByLabelText("Collapsed breadcrumb"),
    ).toHaveTextContent("...");
    expect(
      within(breadcrumb).getByRole("button", { name: "acme" }),
    ).toBeInTheDocument();
    expect(
      within(breadcrumb).getByRole("button", { name: "briefs" }),
    ).toBeInTheDocument();
  });
  ...
  test("filters visible files from the toolbar", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "hero-banner.jpg" }),
    ).not.toBeInTheDocument();
  });
  ...
  test("renames a selected item with a custom dialog", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "hero-renamed.jpg" }),
    ).toBeInTheDocument();
  });
  ...
  test("confirms before deleting selected items", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Delete selected items" }),
    ).toHaveTextContent("hero-banner.jpg");

    await user.click(screen.getByRole("button", { name: "Cancel" }));
    expect(
      screen.getByRole("button", { name: "hero-banner.jpg" }),
    ).toBeInTheDocument();
  ...
  test("surfaces partial bulk delete failures without rolling back successes", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "quarterly-report.pdf" }),
    ).toBeInTheDocument();
  });
  ...
  test("renames inline from keyboard shortcuts", async () => {
    ...
    await user.keyboard("{Enter}");
    expect(
      await screen.findByRole("button", { name: "hero-inline.jpg" }),
    ).toBeInTheDocument();
  ...
  test("keeps inline rename open on adapter conflicts", async () => {
    ...
    Object.defineProperty(adapter, "rename", {
      value: () =>
        Promise.reject(
          new FileBrowserAdapterError("conflict", "Name already exists"),
        ),
    });
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("textbox", { name: "Rename hero-banner.jpg" }),
      ).toHaveValue("quarterly-report.pdf"),
    );
  ...
  test("moves item focus and selection with keyboard arrows", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("gridcell", { name: "assets Folder" })).toHaveFocus(),
    );
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("gridcell", { name: "hero-banner.jpg 15 B" }),
      ).toHaveFocus(),
    );
  ...
  test("moves list-row focus with keyboard arrows", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("row", { name: /assets/ })).toHaveFocus(),
    );
  });
  ...
  test("moves selected items through a destination picker", async () => {
    ...
    expect(
      (await adapter.list("/assets/archive")).items.map((item) => item.path),
    ).toContain("/assets/archive/hero-banner.jpg");
  });
  ...
  test("moves dragged items onto folders when move is supported", async () => {
    ...
    expect((await adapter.list("/assets")).items.map((item) => item.path)).toContain(
  ...
  test("offers cut then paste into another folder", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "hero-banner.jpg" }),
    ).toBeInTheDocument();
    expect((await adapter.list("/")).items.map((item) => item.path)).not.toContain(
      "/hero-banner.jpg",
    );
  });
  ...
  test("resolves upload name conflicts with keep both", async () => {
    ...
    await user.upload(
      ...
      textFile("hero-banner.jpg", "replacement"),
    );
    ...
    expect(
      await screen.findByRole("button", { name: "hero-banner (1).jpg" }),
    ).toBeInTheDocument();
  });
  ...
  test("uploads through the provider transfer manager", async () => {
    ...
    await user.upload(
      screen.getByLabelText("Upload files"),
      textFile("provider-upload.txt", "provider"),
    );
    ...
    expect(
      await screen.findByRole("button", { name: "provider-upload.txt" }),
    ).toBeInTheDocument();
  });
  ...
  test("uploads pasted clipboard images through provider transfers", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "clipboard-shot.png" }),
    ).toBeInTheDocument();
  });
  ...
  test("uploads dropped nested folders through provider transfers", async () => {
    ...
    fireEvent.drop(screen.getByRole("grid", { name: "Files" }), {
      dataTransfer: dataTransferWithEntry(
        fakeDirectoryEntry("photos", [
          fakeFileEntry(textFile("nested-photo.txt", "nested")),
        ]),
      ),
    });
    ...
    expect((await adapter.list("/photos")).items.map((item) => item.path)).toContain(
  ...
  test("detects nested upload conflicts by listing when exists is absent", async () => {
    ...
    await adapter.upload(
      "/photos/nested-photo.txt",
      textFile("nested-photo.txt", "existing"),
    );
    ...
    fireEvent.drop(screen.getByRole("grid", { name: "Files" }), {
      dataTransfer: dataTransferWithEntry(
        fakeDirectoryEntry("photos", [
          fakeFileEntry(textFile("nested-photo.txt", "replacement")),
        ]),
      ),
    });
