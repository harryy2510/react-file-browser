...
export class TransferManager {
  ...
  hasActiveTransfers(): boolean {
    ...
    return snapshot.uploads.some((upload) =>
      ["queued", "uploading"].includes(upload.status),
    ) || snapshot.downloads.some((download) =>
  ...
  resumeRestoredUpload({
    adapter,
    file,
    id,
    onConflict,
  }: ResumeRestoredUploadInput): Promise<void> {
    const upload = this.uploads.get(id);
    if (!upload) {
      return Promise.resolve();
    }
  ...
  private updateUploadProgress(
    ...
    if (
      ...
      upload.bytesPerSecond =
        ((loadedBytes - previous.loadedBytes) / (timeMs - previous.timeMs)) *
        1000;
    }
...
function hasMultipart(adapter: FileBrowserAdapter): adapter is FileBrowserAdapter &
  Required<
    Pick<
      FileBrowserAdapter,
...
async function prepareMultipartDestination(upload: UploadTransfer): Promise<void> {
  ...
  const exists: NonNullable<FileBrowserAdapter["exists"]> = (paths) =>
    adapter.exists?.(paths) ?? Promise.resolve({});
  ...
  if (upload.onConflict !== "replace") {
    throw new FileBrowserAdapterError(
      "conflict",
      `A file browser entry already exists at ${upload.path}`,
    );
  }
}
...
function isExpiredOrStaleDownload(
  download: InternalBulkDownloadJob,
  nowMs: number,
): boolean {
  ...
  return (
    ...
    nowMs - updatedAtMs > DOWNLOAD_RETENTION_MS
  );
}
...
async function createClientZipUrl(
  adapter: FileBrowserAdapter,
  paths: string[],
): Promise<string> {
...
async function collectClientZipEntries(
  adapter: FileBrowserAdapter,
  paths: string[],
): Promise<ClientZipEntry[]> {
