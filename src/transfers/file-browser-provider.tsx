...
export type FileBrowserProviderProps = PropsWithChildren<{
  ...
  resolveRestoredUpload?: (
    ...
    | Omit<ResumeRestoredUploadInput, "id">
    | Promise<Omit<ResumeRestoredUploadInput, "id"> | undefined>
    | undefined;
...
export function FileBrowserProvider({
  ...
  const transferManager = useMemo(
    () => manager ?? new TransferManager(withDefaultStorage(options)),
    [manager, options],
  );
  ...
  return (
    <TransferContext.Provider value={transferManager}>
      {children}
      {showFloatingWidget ? (
        <FloatingTransferWidget manager={transferManager} snapshot={snapshot} />
      ) : null}
      {!resumePromptDismissed && restorableUploads.length > 0 ? (
        <ResumeUploadsPrompt
          count={restorableUploads.length}
          onDismiss={() => {
            transferManager.dismissRestoredUploads(
              restorableUploads.map((upload) => upload.id),
            );
            setResumePromptDismissed(true);
          }}
          onResume={() => {
...
function withDefaultStorage(
  options: TransferManagerOptions | undefined,
...
function FloatingTransferWidget({
  manager,
  snapshot,
}: {
  manager: TransferManager;
  ...
  const groupedUploadIds = new Set(
    uploadGroups.flatMap((group) =>
      group.activeUploads.map((upload) => upload.id),
    ),
  );
  const standaloneUploads = activeUploads.filter(
    (upload) => !groupedUploadIds.has(upload.id),
  );
  ...
  const content = (
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <div className="text-[11px] text-[var(--fb-muted)]">
    ...
    <div>
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="text-[11px] text-[var(--fb-muted)]">
            {formatUploadGroupSummary(group)}
          </div>
        </div>
        <button
    ...
    <div>
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <span className="text-[11px] text-[var(--fb-muted)]">
      ...
      <div
        className={`h-1.5 overflow-hidden rounded-full bg-[var(--fb-surface-2)] ${
          compact ? "mt-1" : "mt-1.5"
        }`}
      >
        <div
          ...
          style={{
            width: `${Math.min(
              100,
              upload.totalBytes
                ? (upload.loadedBytes / upload.totalBytes) * 100
                : 0,
            )}%`,
          }}
        />
...
function getActiveUploadGroups(uploads: UploadTransfer[]): ActiveUploadGroup[] {
  ...
  return Array.from(groups.values()).filter(
    (group) => group.activeUploads.length > 0,
  );
}
...
function formatDownloadReadyLabel(download: BulkDownloadJob): string {
  const expiresIn = download.expiresAt
    ? formatExpiresIn(download.expiresAt)
    : undefined;

  return expiresIn
    ? `Download ready, expires in ${expiresIn}`
    : "Download ready";
}
