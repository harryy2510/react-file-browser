...
export type S3CompatibleFileBrowserAdapterOptions =
...
export class S3CompatibleFileBrowserAdapter implements FileBrowserAdapter {
  ...
  constructor(
    adapterName: CloudAdapterName,
    options: S3CompatibleFileBrowserAdapterOptions = {},
  ) {
    ...
    this.sdk = {
      ...
      multipartPartSize: Math.max(
        1,
        options.multipartPartSize ?? DEFAULT_MULTIPART_PART_SIZE,
      ),
  ...
  private async createMultipartUploadEntry(
    ...
    if (!response.UploadId) {
      throw new FileBrowserAdapterError(
        "not_supported",
        "S3-compatible multipart upload did not return an upload id.",
      );
    }
  ...
  private getMultipartSession(uploadId: string): S3MultipartSession {
    ...
    if (!session) {
      throw new FileBrowserAdapterError(
        "not_found",
        `No multipart upload session exists for ${uploadId}`,
      );
    }
    return session;
  }
...
function toS3AdapterError(error: unknown): FileBrowserAdapterError {
  ...
  if (status === 403 || candidate.name === "AccessDenied") {
    return new FileBrowserAdapterError(
      "access_denied",
      candidate.message ?? "Access denied by the object store.",
      { cause: error },
    );
  }
  if (
    ...
    return new FileBrowserAdapterError(
      "not_found",
      candidate.message ?? "The object does not exist.",
      { cause: error },
    );
  }
  return new FileBrowserAdapterError(
    "not_supported",
    candidate.message ?? "The object store request failed.",
    { cause: error },
  );
}
