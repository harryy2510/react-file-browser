...
export class InMemoryFileBrowserAdapter implements FileBrowserAdapter {
  ...
  readonly bulkDownloadUrl?: NonNullable<
    FileBrowserAdapter["bulkDownloadUrl"]
  >;
  readonly createMultipartUpload?: NonNullable<
    FileBrowserAdapter["createMultipartUpload"]
  >;
  readonly uploadPart?: NonNullable<FileBrowserAdapter["uploadPart"]>;
  readonly completeMultipartUpload?: NonNullable<
    FileBrowserAdapter["completeMultipartUpload"]
  >;
  readonly abortMultipartUpload?: NonNullable<
  ...
  constructor(options: InMemoryFileBrowserAdapterOptions = {}) {
    ...
    if (capabilities.multipart) {
      ...
      this.completeMultipartUpload =
        this.completeMultipartUploadEntry.bind(this);
  ...
  list(
    ...
    if (folder.kind !== "folder") {
      throw new FileBrowserAdapterError(
        "invalid_path",
        `Cannot list a file path: ${normalized}`,
      );
    }
  ...
  signedUrl(path: string): Promise<string> {
    ...
    if (entry.kind !== "file" || !entry.blob) {
      throw new FileBrowserAdapterError(
        "invalid_path",
        `Cannot create a signed URL for a folder: ${entry.path}`,
      );
    }
  ...
  private renameEntry(path: string, newName: string): Promise<FileNode> {
    ...
    if (!trimmedName || trimmedName.includes("/") || trimmedName.includes("\\")) {
      throw new FileBrowserAdapterError(
        "invalid_path",
        `Invalid file browser name: ${newName}`,
      );
    }
  ...
  private copyEntries(from: string[], toDir: string): Promise<void> {
    ...
    for (const sourcePath of from.map(normalizeFileBrowserPath)) {
      ...
      const destinationRoot = joinFileBrowserPath(
        normalizedToDir,
        getFileBrowserBasename(source.path),
      );

      for (const entry of this.collectTree(source.path)) {
        const destinationPath = replaceFileBrowserPathPrefix(
          entry.path,
          source.path,
          destinationRoot,
        );
        ...
        if (this.entries.has(destinationPath) || copies.has(destinationPath)) {
          throw new FileBrowserAdapterError(
            "conflict",
            `A file browser entry already exists at ${destinationPath}`,
          );
        }
        ...
        copies.set(destinationPath, {
          ...
          name:
            entry.path === source.path
              ? getFileBrowserBasename(destinationPath)
              : entry.name,
          modifiedAt: this.timestamp(),
          etag: this.nextEtag(),
        });
  ...
  private async createBulkDownloadUrl(
    paths: string[],
    ...
    for (const path of paths) {
      ...
      for (const entry of tree) {
        ...
        zippable[toArchivePath(entry.path)] = new Uint8Array(
  ...
  private createMultipartUploadEntry(
    path: string,
    size: number,
  ...
  private completeMultipartUploadEntry({
    ...
    NonNullable<FileBrowserAdapter["completeMultipartUpload"]>
  >[0]): Promise<FileNode> {
    return Promise.resolve().then(() => {
      const upload = this.getMultipartUpload(uploadId);
      const blobs: Blob[] = [];
      ...
      for (const part of parts.sort(
        ...
        if (!uploaded || uploaded.etag !== part.etag) {
          throw new FileBrowserAdapterError(
            "not_found",
            `No uploaded part ${part.partNumber} exists for ${uploadId}`,
          );
        }
      ...
      if (blob.size !== upload.size) {
        throw new FileBrowserAdapterError(
          "invalid_path",
          `Multipart upload ${uploadId} size mismatch`,
        );
      }
  ...
  private getMultipartUpload(uploadId: string): InMemoryMultipartUpload {
    ...
    if (!upload) {
      throw new FileBrowserAdapterError(
        "not_found",
        `No multipart upload exists for ${uploadId}`,
      );
    }
    return upload;
  }
  ...
  private assertFolderExists(path: string): void {
    ...
    if (entry.kind !== "folder") {
      throw new FileBrowserAdapterError(
        "invalid_path",
        `Expected a folder at ${entry.path}`,
      );
    }
  }
  ...
  private getEntry(path: string): InMemoryEntry {
    ...
    if (!entry) {
      throw new FileBrowserAdapterError(
        "not_found",
        `No file browser entry exists at ${normalized}`,
      );
    }
