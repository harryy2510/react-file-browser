...
import type { CloudFileBrowserAdapterOptions } from '../cloud-adapter-base'

export type SupabaseStorageBucket = {
	copy?: (fromPath: string, toPath: string) => Promise<SupabaseResult<{ path?: string }>>
...
export class SupabaseStorageFileBrowserAdapter implements FileBrowserAdapter {
	...
	async list(path: string, opts: FileBrowserListOptions = {}): Promise<FileBrowserListResult> {
		...
		return {
			cursor: data.length === this.sdk.pageSize ? String(offset + data.length) : undefined,
			items
		}
	}
	...
	private async createFolderEntry(path: string): Promise<FileNode> {
		...
		await this.unwrap(
			sdk.bucket.upload(`${this.pathToKey(normalized)}/${EMPTY_FOLDER_MARKER}`, '', {
	...
	async delete(paths: string[]): Promise<void> {
		...
		if (!this.sdk.bucket.remove) {
			throw new FileBrowserAdapterError('not_supported', 'Supabase storage remove is not available on this client.')
		}
	...
	async signedUrl(path: string): Promise<string> {
		...
		const data = await this.unwrap(this.sdk.bucket.createSignedUrl(this.pathToKey(path), this.sdk.signedUrlExpiresIn))
	...
	private async statEntry(path: string): Promise<FileNode> {
		...
		if (!entry) {
			throw new FileBrowserAdapterError('not_found', `No Supabase storage entry exists at ${normalized}`)
		}
	...
	private async copyEntries(from: string[], toDir: string): Promise<void> {
		if (!this.sdk!.bucket.copy) {
			throw new FileBrowserAdapterError('not_supported', 'Supabase storage copy is not available on this client.')
		}
	...
	private async collectFolderKeys(folderKey: string, keys: Set<string>): Promise<void> {
		const entries = await this.unwrap(
	...
	private async unwrap<T>(result: Promise<SupabaseResult<T>>): Promise<T> {
		...
		if (data === null) {
			throw new FileBrowserAdapterError('not_found', 'Supabase storage returned no data.')
		}
		return data
	}
}
...
function toSupabaseAdapterError(error: SupabaseErrorLike): FileBrowserAdapterError {
	const status = typeof error.statusCode === 'string' ? Number.parseInt(error.statusCode, 10) : error.statusCode
	if (status === 403 || error.name === 'AccessDenied') {
		return new FileBrowserAdapterError('access_denied', error.message ?? 'Access denied by Supabase storage.', {
			cause: error
		})
	}
	if (status === 404 || error.name === 'NotFound') {
		return new FileBrowserAdapterError('not_found', error.message ?? 'Supabase storage object was not found.', {
			cause: error
		})
	}
	if (status === 409 || error.name === 'Conflict') {
		return new FileBrowserAdapterError('conflict', error.message ?? 'Supabase storage object already exists.', {
			cause: error
		})
	}
	return new FileBrowserAdapterError('not_supported', error.message ?? 'Supabase storage request failed.', {
		cause: error
	})
}
