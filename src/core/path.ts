...
export function joinFileBrowserPath(dir: string, name: string): string {
  ...
  return normalizeFileBrowserPath(
    normalizedDir === ROOT_PATH ? `/${name}` : `${normalizedDir}/${name}`,
  );
}
...
export function assertValidFileBrowserName(name: string): void {
  ...
  if (!trimmed || trimmed.includes("/") || trimmed.includes("\\")) {
    throw new FileBrowserAdapterError(
      "invalid_path",
      `Invalid file browser name: ${name}`,
    );
  }
}

export function isFileBrowserDescendantOrSelf(
  parentPath: string,
...
export function replaceFileBrowserPathPrefix(
  path: string,
  fromPrefix: string,
  toPrefix: string,
): string {
  ...
  return normalizeFileBrowserPath(
    `${normalizedTo}${normalizedPath.slice(normalizedFrom.length)}`,
  );
}
