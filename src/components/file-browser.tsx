...
import Selecto, { type OnSelect } from "react-selecto";
import {
  useFileBrowser,
...
import {
  collectUploadCandidatesFromDataTransfer,
  getFileBrowserUploadCandidates,
  type FileBrowserUploadCandidate,
} from "../core/upload-drop";
import {
  useTransferSnapshot,
  useTransfers,
...
export function FileBrowser({
  ...
  const [bulkFailure, setBulkFailure] =
    useState<FileBrowserBulkActionError | null>(null);
  ...
  const [moveDestinationStatus, setMoveDestinationStatus] =
    useState<MoveDestinationStatus>("idle");
  const [moveDestinationError, setMoveDestinationError] = useState<string | null>(
    null,
  );
  const [contextMenu, setContextMenu] = useState<ContextMenuState | null>(null);
  const [uploadConflictQueue, setUploadConflictQueue] =
    useState<UploadConflictQueue | null>(null);
  ...
  const openPreview = useCallback(
    async (item: FileNode) => {
      ...
      try {
        ...
        setPreview((current) =>
          current?.item.path === item.path
            ? { item, status: "error", error: toErrorMessage(error) }
            : current,
        );
      }
    },
  ...
  const selectItemWithEvent = useCallback(
    (path: string, event: MouseEvent | KeyboardEvent) => {
      ...
      const isFastRepeatClick =
        previousClick?.path === path &&
        clickedAt - previousClick.timeMs <=
          SELECTED_ITEM_DOUBLE_CLICK_WINDOW_MS;
  ...
  const moveKeyboardSelection = useCallback(
    (key: string, extendSelection: boolean) => {
      ...
      const fallbackIndex =
        key === "ArrowUp" || key === "ArrowLeft" || key === "End"
          ? visibleItems.length - 1
          : 0;
      const nextIndex =
        currentIndex === -1
          ? fallbackIndex
          : getNextKeyboardIndex(key, currentIndex, visibleItems.length);
  ...
  useEffect(() => {
    for (const upload of transferSnapshot.uploads) {
      if (
        ...
        !upload.result ||
        refreshedUploadIdsRef.current.has(upload.id)
      ) {
        continue;
      }
  ...
  function startItemDrag(item: FileNode, event: DragEvent) {
    ...
    const paths = browser.selectedPaths.includes(item.path)
  ...
  function showAdjacentPreview(direction: -1 | 1) {
    ...
    const currentIndex = previewFiles.findIndex(
      (item) => item.path === preview.item.path,
    );
    const safeIndex = currentIndex >= 0 ? currentIndex : 0;
    const nextIndex =
      (safeIndex + direction + previewFiles.length) % previewFiles.length;
  ...
  async function downloadSelection() {
    if (
      browser.selectedItems.length === 1 &&
      browser.selectedItems[0].kind === "file"
    ) {
    ...
    const paths =
      browser.selectedPaths.length > 0
        ? browser.selectedPaths
        : browser.items.map((item) => item.path);
  ...
  async function beginUploadCandidates(candidates: FileBrowserUploadCandidate[]) {
    ...
    const { acceptedCandidates, rejectedCandidates } = applyUploadPolicy(
      candidates,
      uploadPolicy,
      browser.currentPath,
    );
    ...
    const group = getUploadTransferGroup(
      browser.currentPath,
      supportedCandidates,
      createdFolders,
    );
  ...
  function rejectUnsupportedFolderUploads(
    candidates: FileBrowserUploadCandidate[],
  ) {
    ...
    const [folderCandidates, flatCandidates] = partitionUploadCandidates(
      candidates,
    ...
    if (!adapter.createFolder) {
      if (folders.length > 0) {
        throw new FileBrowserAdapterError(
          "not_supported",
          "Folder uploads require folder creation support",
        );
      }
      return 0;
    }
    ...
    for (const folder of folders) {
      try {
        await adapter.createFolder(
          joinUploadRelativePath(browser.currentPath, folder),
        );
        createdFolders += 1;
      } catch (error) {
        if (
          error instanceof FileBrowserAdapterError &&
          error.code === "conflict"
        ) {
          continue;
        }
  ...
  async function findUploadConflictPaths(
    candidates: FileBrowserUploadCandidate[],
  ) {
    const paths = candidates.map((candidate) =>
      joinUploadRelativePath(browser.currentPath, candidate.relativePath),
    );
  ...
  function enqueueUploadTransfer(
    ...
    transfers.enqueueUpload({
      adapter,
      destinationPath: joinUploadRelativePath(
        browser.currentPath,
  ...
  return (
    ...
    >
        ...
        <header className="flex min-h-[52px] flex-wrap items-center gap-2 border-b border-[var(--fb-border)] bg-[var(--fb-surface)] px-3 py-2">
          ...
          <div className="ml-auto flex min-w-0 items-center gap-1.5">
            ...
            <select
              ...
              onChange={(event) =>
                browser.setFilterKind(
                  event.target.value as "all" | "files" | "folders",
                )
              }
            ...
            <select
              ...
              onChange={(event) =>
                browser.setSortBy(
                  event.target.value as "name" | "modifiedAt" | "size",
                )
              }
            ...
            <button
              ...
              onClick={() =>
                browser.setSortDirection((current) =>
        ...
        >
          {browser.status === "loading" && browser.items.length === 0 ? (
            ...
            <StateMessage
              ...
              value="Create a folder or upload files to start."
            />
          ) : browser.view === "grid" ? (
            <FileGrid
              browser={browser}
              ...
              onFolderDrop={(item, event) =>
                void moveDraggedItemsToFolder(item, event)
              }
      ...
      {showDetailsPanel ? (
        <DetailsPanel
          item={selected}
          onDownload={() => selected ? void downloadItem(selected) : undefined}
        />
      ) : null}
        ...
        >
          ...
          >
            ...
            <p className="mt-2 text-[12px] text-[var(--fb-muted)]">
              This removes the selected entries from storage.
            </p>
      ...
      {moveDialogOpen ? (
        ...
        >
          ...
          >
            ...
            <p className="mt-2 text-[12px] text-[var(--fb-muted)]">
              Choose a destination folder from the folder tree.
            </p>
            ...
            />
            <div className="mt-3 truncate text-[11px] text-[var(--fb-muted)]">
      ...
      {uploadConflictQueue ? (
        <UploadConflictDialog
          ...
          onResolve={(resolution) => void resolveUploadConflict(resolution)}
        ...
        >
          <div className="w-[min(720px,100%)] overflow-hidden rounded-[var(--fb-radius)] border border-[var(--fb-border)] bg-[var(--fb-surface)] shadow-[0_22px_70px_color-mix(in_oklch,var(--fb-text)_24%,transparent)]">
            ...
            <div className="grid min-h-[260px] place-items-center bg-[var(--fb-bg)] p-8">
              {(preview.item.mimeType?.startsWith("image/") && preview.url) ||
              preview.item.thumbnailUrl ? (
                ...
                <div className="text-center">
                  ...
                  <div className="mt-1 text-[12px] text-[var(--fb-muted)]">
                    {preview.item.mimeType ?? "File"}
                  </div>
                  {preview.status === "loading" ? (
                    <div className="mt-3 text-[12px] text-[var(--fb-muted)]">
                      Loading preview
                    </div>
                  ) : null}
...
function joinUploadRelativePath(currentPath: string, relativePath: string) {
  return relativePath
    ...
    .filter(Boolean)
    .reduce(
      (path, part) => joinFileBrowserPath(path, part),
      currentPath,
    );
}
...
function applyUploadPolicy(
  ...
  for (const candidate of candidates) {
    ...
    if (
      ...
      candidate.file.size > policy.maxFileSizeBytes
    ) {
      reasons.push(
        `File is larger than ${formatBytes(policy.maxFileSizeBytes)}`,
      );
    }

    if (
      policy.allowedMimeTypes?.length &&
      !isAllowedUploadType(candidate.file, policy.allowedMimeTypes)
    ) {
      reasons.push(
        `File type ${candidate.file.type || "unknown"} is not allowed`,
      );
    }

    if (
      policy.remainingQuotaBytes !== undefined &&
      acceptedBytes + candidate.file.size > policy.remainingQuotaBytes
    ) {
      reasons.push(
        `Upload exceeds remaining quota of ${formatBytes(
          policy.remainingQuotaBytes,
        )}`,
      );
    }
...
function FileGrid({
  ...
  function handleMarqueeSelect(event: OnSelect) {
    const inputEvent = event.inputEvent as
      | MouseEvent
      | KeyboardEvent
      | undefined;
    ...
    browser.setSelection(paths, {
      additive: Boolean(
        inputEvent?.shiftKey || inputEvent?.metaKey || inputEvent?.ctrlKey,
      ),
    });
  }
      ...
      {gridElement ? (
        <Selecto
          ...
          selectableTargets={[
            () =>
              Array.from(
                gridElement.querySelectorAll<HTMLElement | SVGElement>(
  ...
  return (
    ...
    >
      <thead className="text-[11px] text-[var(--fb-muted)]">
        <tr>
          <th className="border-b border-[var(--fb-border)] px-3 py-2 font-medium">
            Name
          </th>
          <th className="border-b border-[var(--fb-border)] px-3 py-2 font-medium">
            Size
  ...
  return (
    <div
      ...
      className={`fixed z-50 rounded-[calc(var(--fb-radius)-2px)] border border-[var(--fb-border)] bg-[var(--fb-surface)] p-1 text-[12px] text-[var(--fb-text)] shadow-[0_16px_44px_color-mix(in_oklch,var(--fb-text)_16%,transparent)] ${
        isSheet
          ? "inset-x-3 bottom-3 max-h-[80vh] overflow-auto"
          : "min-w-40"
      }`}
    ...
    >
      {isItemMenu && !readOnly && browser.capabilities.rename ? (
        <ContextMenuButton onClick={() => run(() => onRename(menu.item))}>
          Rename
        </ContextMenuButton>
      ) : null}
      ...
      {isItemMenu && !readOnly && browser.capabilities.copy ? (
        <ContextMenuButton onClick={() => run(onCopy)}>
          Copy
        </ContextMenuButton>
      ) : null}
      {isItemMenu && !readOnly && browser.capabilities.move ? (
        <ContextMenuButton onClick={() => run(onCut)}>
          Cut
        </ContextMenuButton>
      ) : null}
      {isItemMenu ? (
        <>
          <ContextMenuButton onClick={() => run(onDownload)}>
            Download
          </ContextMenuButton>
          {!readOnly ? (
            <ContextMenuButton
          ...
          {!readOnly ? (
            <>
              {browser.capabilities.createFolder ? (
                <ContextMenuButton onClick={() => run(onNewFolder)}>
                  New folder
                </ContextMenuButton>
              ) : null}
              <ContextMenuButton onClick={() => run(onUpload)}>
                Upload
              </ContextMenuButton>
            </>
          ) : null}
          {!readOnly && browser.clipboard ? (
            <ContextMenuButton onClick={() => run(onPaste)}>
              Paste here
            </ContextMenuButton>
          ) : null}
        </>
...
function MoveDestinationPicker({
  ...
  return (
    <div className="mt-3 max-h-64 overflow-auto rounded-[calc(var(--fb-radius)-3px)] border border-[var(--fb-border)] bg-[var(--fb-bg)] p-2">
      ...
      />
      {status === "loading" ? (
        <div className="px-2 py-3 text-[12px] text-[var(--fb-muted)]">
          Loading folders
        </div>
      ) : null}
      {status === "error" ? (
        <div className="px-2 py-3 text-[12px] text-[var(--fb-danger)]">
  ...
  return (
    <button
      ...
      className={`mt-1 flex h-8 w-full items-center gap-2 rounded-[calc(var(--fb-radius)-4px)] px-2 text-left text-[12px] font-medium ${
        active
          ? "bg-[var(--fb-accent-soft)] text-[var(--fb-accent)]"
          : "hover:bg-[var(--fb-surface)]"
      }`}
      onClick={onClick}
...
function ActionBar({
  ...
  return (
    <div
      ...
      role="toolbar"
    >
      <span className="mr-auto text-[12px] font-semibold">
      ...
      <div>
        <div
          className={`font-semibold ${
            tone === "danger" ? "text-[var(--fb-danger)]" : ""
          }`}
        >
          {title}
        </div>
    ...
    >
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <div className="font-semibold text-[var(--fb-danger)]">
            Upload rejected
          </div>
          <ul className="mt-1 space-y-1">
            {rejections.map((rejection) => (
              <li className="min-w-0" key={rejection.relativePath}>
                ...
                <span className="text-[var(--fb-muted)]">
                  {" "}
...
function selectWithEvent(
  ...
  if ("shiftKey" in event && event.shiftKey) {
    ...
    browser.selectedPaths.length === 1 &&
    browser.selectedPaths[0] === path
  ) {
...
function isKeyboardNavigationKey(key: string) {
  return ["ArrowDown", "ArrowLeft", "ArrowRight", "ArrowUp", "End", "Home"].includes(
    key,
  );
}
...
function formatScreenReaderStatus({
  ...
  const folderName =
    currentPath === "/"
      ? "Files"
      : currentPath.split("/").filter(Boolean).at(-1) ?? "Files";
  return `${folderName} ${status}. ${itemCount} ${
